...
public class TriangleSolitaireModel implements MarbleSolitaireModel {
  ...
  // Represents arm thickness which is the total number of rows in the board
  private final int armThickness;
  private final ArrayList<ArrayList<SlotState>> board;

  /**
   * Instantiates a Triangle-style Marble Solitaire Model with a default arm thickness of 3 and an
   * empty center at (0, 0).
   */
  public TriangleSolitaireModel() {
    this.armThickness = 5;
    board = makeBoard(5, 0, 0);
  }
  ...
  public TriangleSolitaireModel(int sRow, int sCol) {
    if (!isValidPosn(5, sRow, sCol)) {
      throw new IllegalArgumentException(
              "Invalid empty cell position (" + sRow + ", " + sCol + ")");
    } else {
      this.armThickness = 5;
      board = makeBoard(5, sRow, sCol);
    }
  }

  /**
   * Instantiates a Triangle-style Marble Solitaire Model with an arm thickness of the passed in
   * dimension and an empty center in the middle at (0, 0)
   *
   * @param dimensions number of slots in the bottom-most row
   * @throws IllegalArgumentException if the dimensions is even or less than 0.
   */
  public TriangleSolitaireModel(int dimensions) {
    if (dimensions < 0) {
      throw new IllegalArgumentException("Arm thickness must be an positive integer");
    } else {
      this.armThickness = dimensions;
      board = makeBoard(dimensions, 0, 0);
    }
  }

  /**
   * Instantiates a Triangle-style Marble Solitaire Model with the given arm thickness and a given
   * empty center at (sRow, sCol).
   *
   * @param dimensions number of slots in the bottom-most row
   * @param sRow       represents the row the center is in
  ...
  public TriangleSolitaireModel(int dimensions, int sRow, int sCol) {
    if (dimensions < 0) {
      throw new IllegalArgumentException("Arm thickness must be an positive integer");
    } else if (!isValidPosn(5, sRow, sCol)) {
      throw new IllegalArgumentException(
              "Invalid empty cell position (" + sRow + ", " + sCol + ")");
    } else {
      this.armThickness = dimensions;
      board = makeBoard(dimensions, sRow, sCol);
    }
  }

  /**
   * Returns whether the given sRow and sCol are valid spaces in a Triangle-style Marble Solitaire
   * Model of the given armThickness
   *
   * @param armThickness the number of slots in the bottom-most row
  ...
  private boolean isValidPosn(int armThickness, int sRow, int sCol) {
    if (sRow < 0 || sRow > armThickness) {
      return false;
    } else if (sCol < 0 || sCol > armThickness) {
      return false;
    } else {
      return true;
    }
  ...
  /**
   * Returns a 2D ArrayList of SlotStates that displays a plus shaped Triangle-style Marble
  ...
  private ArrayList<ArrayList<SlotState>> makeBoard(int armThickness, int sRow, int sCol) {
    ...
    for (int r = 0; r < armThickness; r += 1) {
      ArrayList<SlotState> tempRow = new ArrayList<>();
      for (int c = 0; c < armThickness; c += 1) {
        if (c <= r) {
  ...
  public void move(int fromRow, int fromCol, int toRow, int toCol) throws IllegalArgumentException {
    ...
    if (this.board.get(fromRow).get(fromCol) == SlotState.Marble && this.board.get(toRow).get(toCol)
      ...
      } else if ((Math.abs(fromCol - toCol) == 2 || fromCol == toCol)
              && Math.abs(fromRow - toRow) == 2 && this.board.get(
                      (fromRow + toRow) / 2).get((fromCol + toCol) / 2)
              .equals(SlotState.Marble)) {
        this.board.get(fromRow).set((fromCol + toCol) / 2, SlotState.Empty);
        this.board.get((fromRow + toRow) / 2).set(fromCol, SlotState.Empty);
        this.board.get(toRow).set(toCol, SlotState.Marble);
      } else {
        System.out.println(this.board.get((toRow + fromRow) / 2).get((toCol + fromCol) / 2));
        throw new IllegalArgumentException(
